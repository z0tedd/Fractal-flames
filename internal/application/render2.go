...
func applyTransformations(k int, x, y float64, Coefficient domain.Coeff) (float64, float64) {
	// c, f, b, e := Coefficient.CC, Coefficient.FC, Coefficient.BC, Coefficient.EC
	// ac, dc := Coefficient.AC, Coefficient.DC
				...
				if xRot >= fractal.XMin && xRot <= fractal.XMax && yRot >= fractal.YMin && yRot <= fractal.YMax {
					...
					if x1 >= 0 && x1 < fractal.XRes && y1 >= 0 && y1 < fractal.YRes {
						...
						if point.Value.Counter == 0 {
							point.Color.R = fractal.Coefficients[i].Color.R
							point.Color.G = fractal.Coefficients[i].Color.G
							point.Color.B = fractal.Coefficients[i].Color.B
						} else {
